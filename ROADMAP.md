...
> **Data → ML → Model → API → Database → Testing → Docker → CI/CD → AWS → Nginx → S3 → CloudWatch → Public Demo → Cleanup**
...
- [ ] Synchronize API/model-loading code with the current pipeline artifact
- [ ] Ensure `model_loader.py`
- [ ] Ensure prediction service
- [ ] Ensure prediction endpoint
...
> **S3 as model artifact backup/storage.**
...
> "We kind of did Docker."

We say:

> Docker phase is complete because the application successfully builds and runs inside Docker.
